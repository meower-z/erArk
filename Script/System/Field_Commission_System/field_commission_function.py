    ...
    if demand_or_reward == True and deduction_or_increase == True:
        ...
        # 抓捕
            ...
            if empty_room != "":
                ...
                default.handle_chara_on_line(fugitive_id, fugitive_id, 1, change_data = game_type.CharacterStatusChange(), now_time = cache.game_time)
    ...
    for commision_id in now_ongoing_field_commissions:
        ...
        if game_time.judge_date_big_or_small(cache.game_time, end_time) or cache.debug_mode:
            ...
            for character_id in send_npc_list:
                ...
                # 派遣人员上线
                default.handle_chara_on_line(character_id, character_id, 1, change_data = game_type.CharacterStatusChange(), now_time = cache.game_time)
