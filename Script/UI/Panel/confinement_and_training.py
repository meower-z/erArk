...
def prepare_training():
    ...
    if cache.rhodes_island.pre_training_lubrication and handle_premise.handle_have_body_lubricant(0):
        default.handle_use_body_lubricant(0, 0, add_time=1, change_data=game_type.CharacterStatusChange(), now_time=cache.game_time)
    ...
    for i in range(len(body_item_list)):
        ...
        if cache.rhodes_island.pre_training_tool_dict[i]:
            ...
            # 避孕套
            if i == 13:
                default.handle_wear_condom(0, 0, add_time=1, change_data=game_type.CharacterStatusChange(), now_time=cache.game_time)
