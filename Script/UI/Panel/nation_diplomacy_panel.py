...
def judge_diplomatic_policy():
    ...
    for country_id in cache.rhodes_island.diplomat_of_country:
        ...
        for i in range(diplomatic_policy_data.difficulty):
            common_default.base_chara_experience_common_settle(diplomat_chara_id, 80, target_character_id=None)
            common_default.base_chara_experience_common_settle(diplomat_chara_id, 82, target_character_id=None)
    ...
    def sure_NPC(self, character_id: int, country_id: int):
        ...
        if old_diplomat_id != 0:
            ...
            if not handle_premise.handle_normal_7(old_diplomat_id):
                default.handle_chara_on_line(old_diplomat_id, old_diplomat_id, 1, change_data = game_type.CharacterStatusChange(), now_time = cache.game_time)
        ...
        if character_id != 0 and character_id != old_diplomat_id:
            ...
            if cache.rhodes_island.current_location[0] == country_id:
                ...
                talk.must_show_talk_check(character_id)
                default.handle_chara_off_line(character_id, character_id, 1, change_data = game_type.CharacterStatusChange(), now_time = cache.game_time)
                # 输出外派提示
