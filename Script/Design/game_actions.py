...
"结算"指原有代码里按行动改动角色数值、触发事件的部分。"行为"指角色身上已写好的 Behavior（记录"正在做什么"的对象，见 Script/Modules/scheduler/action.py），
...
- Runtime：本局的执行器。持有一个 Scheduler，为它提供三个回调：轮到 NPC 时调用 npc_ai 选择行动（choose_next）；
  执行一个行动，即把它写成角色的行为并调用原有的结算函数（execute）；每条待办执行前（before_task）：把游戏时间设为待办的时刻，
  跨天则先做每日结算，若待办是"等待玩家输入"则先结束玩家上一个行动，若待办是"NPC 自己选行动"则先运行原有代码里选行动之前的检查。
- advance(minutes)：界面在玩家选好行动后调用，执行玩家的行动并推进时间，直到再次轮到玩家输入。
- 结算代码需要"立刻让某人做某事"时调用的入口：submit_current 把已写在角色身上的行为立即执行；
  wait_on 让一方等待另一方的双人行动；replan 让被别人改写了行为的 NPC 从现在起按新行为执行；
  continue_after 登记玩家本次行动结束后再执行的接续（例如分段寻路的下一段）；reset_character 处理角色加入或离开可行动的干员名单。
- reset：新开局或读档后丢弃执行器。执行器不存进存档，读档后从各角色当前的行为重建。

"强制后续"指结算过程中要求某角色立刻做的行动。它替换该角色原有的待办并标记为立即执行，所以在当前行动结算完成后紧接着执行。
"""
...
class Runtime:
    """本局的执行器：持有调度器，提供它需要的游戏侧回调，并记录结算中登记的强制后续和玩家行动后的接续。"""
