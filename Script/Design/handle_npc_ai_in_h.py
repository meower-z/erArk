...
def judge_character_h_obscenity_unconscious(character_id: int, pl_start_time: datetime.datetime) -> int:
    ...
    if handle_premise.handle_self_orgasm_edge_relase(character_id):
        default.handle_self_orgasm_edge_off(character_id, None, 1, change_data=game_type.CharacterStatusChange(), now_time=datetime.datetime(1, 1, 1))
    # 将时停解放状态改为False
...
def recover_from_unconscious_h(character_id: int, target_character_id: int, info_text: str = ""):
    ...
    if handle_premise.handle_group_sex_mode_on(character_id):
        # 清空玩家的群交模板数据
        default.handle_clear_group_sex_template(character_id, None, 1, game_type.CharacterStatusChange(), datetime.datetime(1, 1, 1))
        # 关闭群交状态
        default.handle_group_sex_mode_off(character_id, None, 1, game_type.CharacterStatusChange(), datetime.datetime(1, 1, 1))
        # 暂存玩家的行为
    ...
    if response == UnconsciousHResponse.CONTINUE_H:
        default.handle_h_flag_to_1(target_data.cid, None, 1, game_type.CharacterStatusChange(), cache.game_time)
        character_data.behavior.behavior_id = constant.Behavior.WAIT
        character_data.state = constant.CharacterStatus.STATUS_WAIT
        # 对方的行为时间设为10分钟
...
def handle_unconscious_h_response(character_id: int, target_character_id: int, can_continue: bool = True) -> int:
    ...
    # 因此先把目标指向自己再发奖
        ...
        default.handle_self_h_state_reset(target_character_id, None, 1, game_type.CharacterStatusChange(), cache.game_time)
        default_cloth.handle_self_cloth_back(target_character_id, None, 1, game_type.CharacterStatusChange(), cache.game_time)
