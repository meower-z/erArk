...
def character_eat_rand_food(character_id: int):
    ...
    if not len(now_food_list):
        ...
        handle_hunger_point_zero(character_id, None,add_time=1,change_data=game_type.CharacterStatusChange(),now_time=cache.game_time)
        handle_eat_food_flag_to_0(character_id, None,add_time=1,change_data=game_type.CharacterStatusChange(),now_time=cache.game_time)
        character_data.target_character_id = character_id
        character_data.behavior.behavior_id = constant.Behavior.SHARE_BLANKLY
        character_data.behavior.duration = 1
        character_data.state = constant.CharacterStatus.STATUS_WAIT
