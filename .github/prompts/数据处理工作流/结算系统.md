...
行为入口为 `handle_settle_behavior(character_id, target_character_id, now_time, event_flag=1)`。普通效果使用以下五个必填参数，`character_id` 表示执行者，`target_character_id` 必须是具体角色 ID，0 表示玩家。直接调用只处理自身的效果时，可将两个 ID 都传为该角色。二段效果沿用独立接口。
