...
行为入口为 `handle_settle_behavior(character_id, target_character_id, now_time, event_flag=1)`。普通效果使用以下五个必填参数，`character_id` 表示执行者，`target_character_id` 表示交互目标，0 表示玩家。二段效果沿用独立接口。
