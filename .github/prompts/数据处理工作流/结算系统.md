...
行为入口为 `handle_settle_behavior(character_id, target_character_id, now_time, event_flag=1)`，目标由调用方传入具体角色 ID。普通效果使用以下五个必填参数，`character_id` 表示执行者。`target_character_id` 传具体角色 ID，0 表示玩家；只处理自身的效果可以显式传 `None`，表示目标不适用。普通效果不解析 `"CURRENT_TARGET"`，该字符串仅供通用辅助函数读取交互对象。二段效果沿用独立接口。

`settle_effect_list` 按配置顺序传递目标。选目标效果写入持久交互目标并返回目标id；其他普通效果返回 `None`。普通效果及嵌套结算改变持久目标后，后续效果使用新目标。
...
def handle_xxx(
    ...
    target_character_id,        # 目标角色ID；无需目标时传None
    add_time: int,             # 增加的时间
