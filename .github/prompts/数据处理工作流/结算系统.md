...
普通效果按是否需要目标使用四参数或五参数签名。无需目标的效果不声明 `target_character_id`：
...
`add_settle_behavior_effect` 在注册时检查参数名和顺序，为四参数效果添加忽略目标的调用入口。注册表统一接收五参数；装饰器返回原函数，所以直接调用效果时按函数声明传参。两种入口都保留效果的返回值，供后续效果接收选中的目标。
...
下面是需要交互目标的效果模板；无需目标时去掉 `target_character_id` 参数及其说明。
