...
[Scheduler](../../Script/Modules/scheduler/scheduler.py) 保存各角色接下来要做的事，按时间依次执行，直到轮到玩家选择行动。
...
[game_actions](../../Script/Design/game_actions.py) 创建调度器时，把 `choose_next` 和 `execute` 两个函数交给它。
